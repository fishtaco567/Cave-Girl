...
namespace SharpNoise
{
    public class PerlinNoise
    {
        ...
        public void GetNoise1D(float xStart, float step, int length, float[] array) {
            if(array.Length < length) {
                UnityEngine.Debug.LogError("Array too short");
            ...
            for(int i = 0; i < length; i++) {
                ...
                array[i] = Lerp(c0, c1, Fade(x)); //Interpolate across the X axis
            }
        }

        public void GetNoise2D(float xStart, float yStart, float stepX, float stepY, int lenX, int lenY, float[] array) {
            if(array.Length < lenX * lenY) {
                UnityEngine.Debug.LogError("Array too short");
            ...
            for(int i = 0; i < lenX; i++) {
                for(int j = 0; j < lenY; j++) {
                    var x = xStart * i * stepX;
                    var y = yStart * i * stepY;
                    ...
                    array[i + j * lenX] = Lerp(x0, x1, Fade(y)); //Interpolate across the Y axis
                }
            }
        }
